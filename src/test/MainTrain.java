...
public class MainTrain {
	...
	public static void testBoard() {
		...
		if(b.tryPlaceWord(bit4)!=15)
			System.out.println("BEWARE SHOULD BE (-15)");

		Word bit5=new Word(get("COOL"), 13, 4, false);
		if(b.tryPlaceWord(bit5)!=26)
			System.out.println("cool SHOULD BE (-15)");

		Word bit6=new Word(get("P_L_Y"), 10, 7, true);
		if(b.tryPlaceWord(bit6)!=79)
			System.out.println("Boly SHOULD BE (-15)");

		Word bit8=new Word(get("ALSTU"), 7, 0, false);
		if(b.tryPlaceWord(bit8)!=39)
			System.out.println("ALSTU SHOULD BE (-15)");

		Word bit7=new Word(get("_OAF"), 7, 0, true);
		if(b.tryPlaceWord(bit7)!=21)
			System.out.println("Aoaf SHOULD BE (-15)");

		Word bit9=new Word(get("COOL"), 11, 0, true);
		if(b.tryPlaceWord(bit9)!=144)
			System.out.println("Aoafcool SHOULD BE (-15)");

		Word bit10=new Word(get("GJRWQXY"), 0, 0, true);
		if(b.tryPlaceWord(bit10)!=1539)
			System.out.println("GJRWQXY SHOULD BE (-15)");

		Word bit12=new Word(get("AETIN"), 8, 10, false);
		if(b.tryPlaceWord(bit12)!=14)
			System.out.println("AETAN SHOULD BE (-15)");

		Word bit11=new Word(get("__"), 7, 7, false);
		if(b.tryPlaceWord(bit11)!=0)
			System.out.println("problem with board legal");

		Word bit13=new Word(get("_I"), 1, 0, false);
		if(b.tryPlaceWord(bit13)!=18)
			System.out.println("JI SHOULD BE (-15)");
	}
